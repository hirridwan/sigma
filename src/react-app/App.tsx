  ...
  return (
    <header className="sticky top-0 z-40 bg-blue-900 text-white shadow-md">
      <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
        <button type="button" onClick={() => navigate("/")} className="flex items-center gap-2 text-xl font-bold transition hover:opacity-90">
    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      <nav className="border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-2 text-xl font-bold text-blue-900"><span className="text-blue-600">▣</span>SIGMA</div>
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-1 items-center px-6 py-12 lg:py-0">
        <div className="grid w-full grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <div className="space-y-6 text-left">
            <span className="inline-block rounded-full border border-blue-200 bg-blue-50 px-4 py-1.5 text-sm font-semibold text-blue-700 shadow-sm">Didukung oleh Artificial Intelligence</span>
            <h1 className="text-4xl font-extrabold leading-tight tracking-tight text-slate-900 md:text-5xl lg:text-6xl">Buat Modul Ajar Terstruktur Hanya dalam <span className="mt-2 block text-blue-600">Hitungan Detik</span></h1>
            <p className="max-w-lg text-base leading-relaxed text-slate-600 md:text-lg">Permudah penyusunan perangkat pembelajaran Kurikulum Merdeka secara otomatis, rapi, dan siap diunduh dalam format Word atau PDF.</p>
            <div className="pt-4">
              <button type="button" onClick={() => navigate("/modul-ajar")} className="inline-flex items-center gap-3 rounded-2xl bg-blue-600 px-8 py-4 text-lg font-bold text-white shadow-lg shadow-blue-600/20 transition hover:-translate-y-1 hover:bg-blue-700">Mulai Buat Modul Sekarang →</button>
            </div>
          </div>
      ...
      <footer className="border-t border-slate-200 bg-white py-6 text-center text-sm font-medium text-slate-500">© 2026 SIGMA. Semua Hak Dilindungi.</footer>
    </div>
  );
}
...
function Feature({ icon, title, description }: { icon: string; title: string; description: string }) {
  return (
    <div className="flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      ...
      <div><div className="mb-1 text-lg font-bold text-blue-900">{title}</div><p className="text-sm leading-relaxed text-slate-600">{description}</p></div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto mt-4 max-w-5xl space-y-6 p-6">
        ...
        <form id="generator-form" onSubmit={generateModule} className="space-y-6">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            ...
            <div className="mb-6 grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <SelectInput label="Jenis Asesmen" value={form.jenis_asesmen} onChange={(v) => update("jenis_asesmen", v)} options={["Asesmen of/as/for Learning","Diagnostik, Formatif, Sumatif"]} placeholder="Pilih Jenis Asesmen" required fullWidth />
            </div>

            <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
              <div className="mb-3 flex items-center justify-between gap-3"><h3 className="flex items-center gap-2 text-sm font-bold text-slate-800">🔗 Sumber Referensi (Opsional)</h3><span className="text-xs text-slate-400">Dipakai sebagai konteks AI</span></div>
        ...
        {resultMarkdown && (
          <section id="hasil-container" className="mb-12 flex min-h-[500px] flex-col rounded-2xl border border-blue-100 bg-white shadow-lg">
            <div className="flex flex-col items-start justify-between gap-4 rounded-t-2xl border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white p-6 lg:flex-row lg:items-center">
              <div><h2 className="flex items-center gap-2 text-xl font-extrabold text-blue-900">📄 Pratinjau Modul</h2><p className="mt-1 text-xs text-slate-500">Hasil dapat diedit dengan mengubah input lalu Regenerate.</p></div>
              <div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-3 lg:w-auto">
                <button type="button" onClick={editInput} className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50">Edit Input</button>
                <button type="button" onClick={() => void generateModule(undefined, true)} disabled={isRegenerating} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:opacity-60">Regenerate</button>
                <button type="button" onClick={copyMarkdown} className="rounded-xl bg-slate-800 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-900">Salin Markdown</button>
                <button type="button" onClick={downloadWord} className="rounded-xl bg-green-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-green-700">Word</button>
                <button type="button" onClick={downloadPdf} className="rounded-xl bg-red-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-red-700">PDF</button>
              </div>
            </div>
            <div id="hasil-content" className="document-preview max-w-none rounded-b-2xl bg-white p-8 md:p-12">
...
function SectionTitle({ number, title }: { number: string; title: string }) {
  return <h2 className="mb-5 flex items-center gap-2 text-lg font-extrabold text-blue-900"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-100 text-sm text-blue-600">{number}</span>{title}</h2>;
}
...
function TextInput({ label, value, onChange, placeholder, type = "text", fullWidth = false, required = false, inputMode }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string; fullWidth?: boolean; required?: boolean; inputMode?: "numeric" | "text" | "url" }) {
  return <div className={fullWidth ? "md:col-span-2" : ""}><label className="mb-1.5 block text-sm font-semibold text-slate-700">{label}</label><input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} required={required} inputMode={inputMode} className="w-full rounded-xl border border-slate-300 px-4 py-2.5 outline-none transition-shadow focus:border-blue-500 focus:ring-2 focus:ring-blue-500" /></div>;
}
...
function TextArea({ label, value, onChange, fullWidth = false, required = false }: { label: string; value: string; onChange: (value: string) => void; fullWidth?: boolean; required?: boolean }) {
  return <div className={fullWidth ? "md:col-span-2" : ""}><label className="mb-1.5 block text-sm font-semibold text-slate-700">{label}</label><textarea value={value} onChange={(e) => onChange(e.target.value)} rows={3} required={required} className="w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition-shadow focus:border-blue-500 focus:ring-2 focus:ring-blue-500" /></div>;
}
